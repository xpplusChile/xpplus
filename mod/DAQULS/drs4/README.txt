...
get_data(channel=1,numero_tri="NULL",run_time="NULL",sampling=5e9,mode="on", rang=0, trig_source="CH1", trig_level=-0.1,trig_pol="falling",trig_delay=150e-9) los valores distintos a los vistos con load_waveform() son numero_tri= que define el numero de triggers que quieres obtener y run_time= que define por cuanto tiempo en segundos quieres obtener triggers y guardar datos, esta funcion devuelve un archivo root en el cual se guarda el evento evn(la informacion de este parametro es el tiempo en que se tomo la medicion o el numero de trigger asociado al evento y esto depende de cual es el parametro que define el fin de la funcion), el tiempo y el voltaje asociado a los canales.
