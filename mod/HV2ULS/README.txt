Este modulo tiene caracteristicas muy similares a HVULS siendo las diferencias que se añade la funcion HV_Recconect() la cual es usada en las funcioenes
de prendido, apagado y de configuracion, tambien incluye otras diferencias en el el output de los parametros de monitoreo y las funciones de prendido 
y apagado.Ahora se explica a continuacion el como usar el modulo asumiendo que se desconoce HVULS:
...
HV_Recconect() 
 Desconecta y vuelve a conectar (la fuente de alto voltaje) al computador para evitar ciertos problemas en al momento de intentar monitorear los distintos 
 parametros posibles, el motivo por el cual suceden estos errores es desconocido pero en caso de que al usar las diferentes funciones la fuente de alto
 voltaje deje de funcionar como se espera hay que utilizar esta funcion
...
HV_Conf(CH=0, vset=1700, iset=400 , rup=30 , rdown=50 , trip= 2  ,imonrange="high", powerdown="kill") 
 ...
 Se agrego HV_Recconect() al final porque en caso de no hacerlo la fuente de alto voltaje reacciona mal a los siguientesc comandos
...
Esto ya no deberia ser necesario desde que se implemento HV_Recconect() 
